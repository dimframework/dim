...
func (c *MigrateCommand) DefineFlags(fs *flag.FlagSet) {
	...
	fs.StringVar(&c.table, "table", DefaultMigrationsTable, "Migrations history table, optionally schema-qualified (e.g. myschema.migrations)")
}
...
func (c *MigrateCommand) Execute(ctx *CommandContext) error {
	...
	table, err := resolveMigrationsTable(c.table)
	if err != nil {
		return err
	}
	...
	if c.verbose {
		...
		fmt.Printf("Recording history in %s\n", table)
	}

	// Sumber migrasi: SetMigrationSource bila dipasang, selain itu gabungan
	// migrasi framework + migrasi terdaftar, terurut berdasarkan Version
	migrations := migrationsFromSource()

	if c.verbose {
		fmt.Printf("Found %d total migrations\n", len(migrations))
	}

	if err := RunMigrationsIn(db, table, migrations); err != nil {
...
func (c *MigrateRollbackCommand) Execute(ctx *CommandContext) error {
	...
	//
	// Cakupannya adalah tabel pencatat ini saja. Dengan pencatat per schema,
	// `-step 2` berarti dua migrasi terakhir milik modul itu, bukan dua baris
	// terakhir dari riwayat gabungan seluruh modul.
	query := fmt.Sprintf(`SELECT version, name FROM %s ORDER BY version DESC LIMIT $1`, table)
	...
	for _, migration := range migrationsFromSource() {
	...
	for _, migration := range migrationsToRollback {
		...
		if err := RollbackMigrationIn(db, table, migration); err != nil {
...
func (c *MigrateListCommand) DefineFlags(fs *flag.FlagSet) {
	fs.StringVar(&c.table, "table", DefaultMigrationsTable, "Migrations history table, optionally schema-qualified (e.g. myschema.migrations)")
}
...
func (c *MigrateListCommand) Execute(ctx *CommandContext) error {
	...
	// Sumber migrasi: SetMigrationSource bila dipasang, selain itu gabungan
	// migrasi framework + migrasi terdaftar, terurut berdasarkan Version
	allMigrations := migrationsFromSource()
	...
	query := fmt.Sprintf(`SELECT version, name, applied_at FROM %s ORDER BY version`, table)
	rows, err := db.Query(context.Background(), query)
	if err != nil {
		// Table might not exist yet
		fmt.Printf("⚠ Migrations table %s does not exist yet. Run 'migrate' first.\n", table)
	} else {
		defer rows.Close()
		for rows.Next() {
			var version int64
